...
class TrajectoryCollector:
    ...
    def preprocess_single_sample(
        ...
        row_dict.update({
            ...
            # Preserve the exact user-side text used to construct this turn's
            # prompt. Offline URCR diagnostics need this boundary without
            # reconstructing it from a decoded and re-tokenized response.
            'turn_context_text': obs_content,
    ...
    def vanilla_multi_turn_loop(
        ...
        for _step in range(self.config.env.max_steps):
            ...
            for key in ("reward_model", "metadata", "extra_info"):
            ...
            if 'search_feedback' in infos[0]:
                batch.non_tensor_batch['search_feedback'] = np.array([info.get('search_feedback', '') for info in infos], dtype=object)
